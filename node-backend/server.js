...
async function ensureMongo() {
	...
	if (!mongoConnection) {
		mongoConnection = mongoose.connect(process.env.MONGO_URI, { serverSelectionTimeoutMS: 2500 })
			.then(() => { mongoReady = true; return true; })
...
app.get('/health', async (_, res) => {
	await ensureMongo();
	res.json({ status: 'ok', database: mongoReady ? 'mongodb' : 'memory-development' });
});
...
if (!process.env.VERCEL) {
	...
	app.listen(PORT, () => console.log(`Node API listening on http://127.0.0.1:${PORT} (${mongoReady ? 'MongoDB' : 'memory development mode'})`));
}
